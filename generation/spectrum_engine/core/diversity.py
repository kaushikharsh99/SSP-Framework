...
logger = logging.getLogger("spectrum-engine.core.diversity")
...
class BaseDiversityCalculator(ABC):
    ...
    @abstractmethod
    def calculate(self, responses: List[ResponseRecord]) -> Dict[str, Any]:
        """Calculates diversity metrics for a set of candidate responses.
        
        Args:
            responses: List of ResponseRecord objects generated for a single prompt.
...
class LexicalDiversityCalculator(BaseDiversityCalculator):
    ...
    def calculate(self, responses: List[ResponseRecord]) -> Dict[str, Any]:
        ...
        # 3. Token-Level Length Statistics (if token_count is populated)
        token_lengths = [r.token_count for r in responses if r.token_count > 0]
        if token_lengths:
